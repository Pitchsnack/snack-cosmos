...
import { reassignDealOwner, reassignDealAiOwner } from "@/lib/deal-ownership.functions";
...
export function DealOwnershipCard({
  dealId, tenantId, owner, aiOwner,
}: { dealId: string; tenantId: string; owner: Person; aiOwner: Person }) {
  const { isControl, has } = usePermissions();
  const canManage = isControl || has("users.assign_role");
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <OwnerBlock title="Owning Agent" person={owner} canManage={canManage} dealId={dealId} tenantId={tenantId} userType="Human" />
      <OwnerBlock title="Owning AI Agent" person={aiOwner} canManage={canManage} dealId={dealId} tenantId={tenantId} userType="AI" />
    </div>
  );
}

function OwnerBlock({
  title, person, canManage, dealId, tenantId, userType,
}: {
  title: string; person: Person; canManage: boolean;
  dealId: string; tenantId: string; userType: "Human" | "AI";
}) {
  return (
    <div className="rounded-lg border border-border bg-card p-5 shadow-card">
      ...
      {canManage && (
        <div className="mt-3">
          <ReassignDialog dealId={dealId} tenantId={tenantId} userType={userType} label={person ? "Reassign" : "Assign"} />
        </div>
      )}
    </div>
  );
...
function ReassignDialog({
  dealId, tenantId, userType, label,
}: { dealId: string; tenantId: string; userType: "Human" | "AI"; label: string }) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState("");
  const qc = useQueryClient();
  const enabled = useHasSession();
  const fetchUsers = useServerFn(listAssignableUsers);
  const reassign = useServerFn(reassignDealOwner);
  const reassignAi = useServerFn(reassignDealAiOwner);
  ...
  const m = useMutation({
    mutationFn: async () => {
      if (userType === "Human") return reassign({ data: { dealId, owningAgentUserId: selected } });
      return reassignAi({ data: { dealId, owningAiAgentId: selected } });
    },
    onSuccess: () => {
