...
import { reassignOwner, reassignAiOwner, listAssignableUsers } from "@/lib/startup-ownership.functions";
import { useHasSession } from "@/hooks/use-has-session";
import { usePermissions } from "@/hooks/use-session-context";

type Person = { id: string; email: string; first_name: string | null; last_name: string | null } | null;

export function StartupOwnershipCard({
  startupId, tenantId, owner, aiOwner,
}: { startupId: string; tenantId: string; owner: Person; aiOwner: Person }) {
  const { isControl, has } = usePermissions();
  const canManage = isControl || has("users.assign_role"); // master agent / tenant admin
  return (
...
function OwnerBlock({
  title, person, canManage, startupId, tenantId, userType,
}: {
  title: string; person: Person; canManage: boolean;
  startupId: string; tenantId: string; userType: "Human" | "AI";
}) {
  return (
...
function ReassignDialog({
  startupId, tenantId, userType, label,
}: { startupId: string; tenantId: string; userType: "Human" | "AI"; label: string }) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState("");
  const qc = useQueryClient();
  const enabled = useHasSession();
  const fetchUsers = useServerFn(listAssignableUsers);
  const reassign = useServerFn(reassignOwner);
  const reassignAi = useServerFn(reassignAiOwner);
  ...
  const m = useMutation({
    mutationFn: async () => {
      if (userType === "Human") return reassign({ data: { startupId, owningAgentUserId: selected } });
      return reassignAi({ data: { startupId, owningAiAgentId: selected } });
    },
    onSuccess: () => {
