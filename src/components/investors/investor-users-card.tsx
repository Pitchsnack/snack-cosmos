...
import { assignInvestorUser, removeInvestorUser } from "@/lib/investor-users.functions";
...
export function InvestorUsersCard({
  investorId, tenantId, assignments,
}: { investorId: string; tenantId: string; assignments: Assignment[] }) {
  const { isControl, has } = usePermissions();
  const canManage = isControl || has("users.assign_role");
  const qc = useQueryClient();
  const remove = useServerFn(removeInvestorUser);
  const removeM = useMutation({
    mutationFn: (assignmentId: string) => remove({ data: { investorId, assignmentId } }),
    onSuccess: () => { toast.success("Removed"); qc.invalidateQueries({ queryKey: ["investor", investorId] }); },
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-5 shadow-card">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Investor Users</div>
          <p className="mt-1 text-xs text-muted-foreground">Partners, principals, analysts with portal access.</p>
        </div>
        {canManage && <AssignDialog investorId={investorId} tenantId={tenantId} />}
      </div>
...
function AssignDialog({ investorId, tenantId }: { investorId: string; tenantId: string }) {
  ...
  const assign = useServerFn(assignInvestorUser);
  ...
  const m = useMutation({
    mutationFn: () => assign({ data: { investorId, userId, role } }),
    onSuccess: () => {
      toast.success("User assigned");
      qc.invalidateQueries({ queryKey: ["investor", investorId] });
      setOpen(false); setUserId(""); setRole("Partner");
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader><DialogTitle>Assign an investor user</DialogTitle></DialogHeader>
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={role} onChange={(e) => setRole(e.target.value)} placeholder="Partner, Principal, Analyst…" />
          </div>
        </div>
        <DialogFooter>
