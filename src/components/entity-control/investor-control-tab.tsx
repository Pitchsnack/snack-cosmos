...
import { useControlFacets, useControlInvestors, useSetDirectoryState } from "@/hooks/use-entity-control";
import { useDebounced } from "@/hooks/use-debounced";
import type { ControlListParams } from "@/lib/entity-control/types";

export function InvestorControlTab() {
  ...
  const params: ControlListParams = useMemo(
    ...
    [page, pageSize, debouncedQ, status, type, stage, sector, country, updated],
  );
  const { data, isLoading, isFetching } = useControlInvestors(params);
  ...
  const bulk = async (state: "published" | "unpublished") => {
    await setState.mutateAsync({ entity: "investor", ids: selected, state });
    setSelected([]);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
        ...
        Published investors are visible in the Investor Directory. Unpublished investors remain
        internal to Control.
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex min-w-[18rem] flex-1 items-center gap-2 rounded-md bg-muted/60 px-3">
          ...
          <Input
            ...
            placeholder="Search investor, fund, website, sector…"
        ...
        <FilterSelect
          label="Investor Type"
          value={type}
          options={(facets.data?.investorTypes ?? []).map((v) => ({ value: v, label: v }))}
          onChange={(v) => { setType(v); setPage(1); }}
        />
        <FilterSelect
          label="Stage Focus"
          value={stage}
          options={(facets.data?.stages ?? []).map((v) => ({ value: v, label: v }))}
        ...
        <FilterSelect
          label="Sector Focus"
          value={sector}
          options={(facets.data?.sectors ?? []).map((v) => ({ value: v, label: v }))}
        ...
        <FilterSelect
          label="HQ / Location"
          value={country}
          options={(facets.data?.investorCountries ?? []).map((v) => ({ value: v, label: v }))}
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        <div className="grid grid-cols-[2.5rem_minmax(0,2.4fr)_minmax(0,1fr)_minmax(0,1.2fr)_minmax(0,1.2fr)_minmax(0,1fr)_8rem_11rem] items-center gap-3 border-b border-border px-4 py-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
          <span />
        ...
        {isLoading && rows.length === 0 ? (
          ...
          rows.map((r) => (
            <div
              key={r.id}
              className="grid h-14 grid-cols-[2.5rem_minmax(0,2.4fr)_minmax(0,1fr)_minmax(0,1.2fr)_minmax(0,1.2fr)_minmax(0,1fr)_8rem_11rem] items-center gap-3 border-b border-border px-4 text-sm last:border-0 hover:bg-muted/40"
            >
              ...
              <span className="truncate text-muted-foreground">{r.investor_type ?? "—"}</span>
              <span className="truncate text-muted-foreground">{r.stages.join(", ") || "—"}</span>
              <span className="truncate text-muted-foreground">{r.sectors.join(", ") || "—"}</span>
