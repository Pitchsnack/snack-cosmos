...
export function InvestorControlTab() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const n = await del.mutateAsync({ entity: "investor", ids: pendingDelete });
      toast.success(`Deleted ${n} investor${n === 1 ? "" : "s"}.`);
  ...
  return (
    <div className="space-y-4">
      ...
      <DeleteEntityDialog
        ...
        title={`Delete ${pendingDelete?.length ?? 0} investor${(pendingDelete?.length ?? 0) === 1 ? "" : "s"}?`}
        description="This permanently removes the record from Control and the Investor Directory. This action cannot be undone."
